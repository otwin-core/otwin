...
A component is a class that declares terminals, parameters and constitutive
...
class BatteryCell(Composite):
    ...
    def __init__(self, capacity=3600.0, resistance=0.05, *, voltage=3.7, name=None):
        ...
        self.link(store.p, internal.p)        # internal wiring
        self.expose("p", internal.n)          # the terminals the outside sees
        self.expose("n", store.n)

cell, load, gnd = BatteryCell(name="cell"), Resistor(10.0, name="load"), Ground()
s = otwin.System(cell, load, gnd).connect(cell.p, load.p).connect(cell.n, load.n, gnd.terminal)
m = otwin.compile(s)
print(m.state_names)
...
`add` takes the parts, `link` joins inner terminals, `expose` publishes an
inner terminal under an outer name. The compiler flattens the composite; the
...
- terminals, with `self.terminal("p")`; the domain defaults to the class's;
- parameters, with `self.param(name, value, unit, positive=..., nonneg=...)`,
...
| `TwoPortBranch` | transformer or gyrator between two terminal pairs | `kind`, `a2`, `b2`, `ratio` |
...
class Supercapacitor(Component):
    ...
    def __init__(self, capacitance=1.0, slope=0.1, *, voltage=0.0, name=None):
        ...
        self.terminal("p")
        self.terminal("n")
        self.C0 = self.param("capacitance", capacitance, "F")
        self.k = self.param("slope", slope, "1/V", positive=False)
...
m = otwin.compile(otwin.System(sc, r, gnd).connect(sc.p, r.p).connect(sc.n, r.n, gnd.terminal))
...
The compiler differentiated the energy to get the terminal voltage
...
- An across storage pins the potential across its terminals, so two of them
