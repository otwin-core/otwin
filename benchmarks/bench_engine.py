...
from otwin.model import dc_motor, integrate_phs, mass_spring_damper, water_tank  # noqa: E402
...
def main() -> None:
    ...
    print(f"otwin {otwin.__version__}  engine={'yes' if engine_available() else 'no'}  {platform.platform()}")
    print(f"{'model':<20} {'steps':>7} {'0.4 python':>12} {'numpy':>10} {'rust mid':>10} {'rust rk4':>10} {'compile':>9} {'batch/run':>10}")
    for name, (old_fn, new_fn, x0, t, u_const) in CASES.items():
        old = old_fn()
        u = None if u_const is None else np.full((t.size, 1), u_const)
        t_old = best_of(lambda: integrate_phs(old, x0, t, u if u is not None else np.zeros((t.size, 1))))
        t0 = time.perf_counter()
        model_np = otwin.compile(new_fn(), backend="numpy")
        t_compile = time.perf_counter() - t0
        inputs = None if u_const is None else {model_np.input_names[0]: u_const}
        t_np = best_of(lambda: model_np.simulate(t=t, x0=x0, inputs=inputs, record=False), n=1)
        row = {"steps": int(t.size - 1), "python_0_4_s": t_old, "numpy_s": t_np, "compile_s": t_compile}
        t_rs = t_rk4 = t_batch = float("nan")
        if engine_available():
            model_rs = otwin.compile(new_fn(), backend="rust")
            t_rs = best_of(lambda: model_rs.simulate(t=t, x0=x0, inputs=inputs, record=False))
            t_rk4 = best_of(lambda: model_rs.simulate(t=t, x0=x0, inputs=inputs, solver="rk4", record=False))
            ...
            t_batch = best_of(lambda: model_rs.simulate_batch(x0s, tb, inputs=ub), n=2) / 100
            row.update({"rust_midpoint_s": t_rs, "rust_rk4_s": t_rk4, "rust_batch_per_run_s": t_batch,
                        "batch_steps": int(tb.size - 1)})
        results["cases"][name] = row
        print(f"{name:<20} {t.size - 1:>7} {t_old:>11.3f}s {t_np:>9.3f}s {t_rs:>9.4f}s {t_rk4:>9.4f}s {t_compile:>8.3f}s {t_batch:>9.5f}s")
