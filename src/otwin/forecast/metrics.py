...
def rmse(
    ...
    Example:
        ...
        >>> round(float(rmse(y_true, y_pred)), 6)
        0.1
    """
    error = y_true - y_pred
...
def nrmse(
    ...
    Example:
        ...
        >>> round(float(nrmse(y_true, y_pred, normalization='range')), 6)
        0.025
    """
...
def mase(
    ...
    Example:
        ...
        >>> round(float(mase(y_true, y_pred, y_train)), 6)
        0.1
    """
    # Model error
...
def theil_u(
    ...
    Example:
        >>> y_true = np.array([1, 2, 3, 4, 5])
        >>> y_pred = np.array([1.1, 2.1, 2.9, 4.1, 5.1])
        >>> round(float(theil_u(y_true, y_pred)), 6)
        0.040825
    """
    # Model RMSE
    model_rmse = rmse(y_true, y_pred)

    # Naive forecast: persistence (last known value), falling back to the
    # first test value when no history was supplied.
    last_value = y_train[-1] if y_train is not None else y_true[0]
...
def crps(
    ...
    for i in range(n):
        ...
        term2 = 0.5 * np.mean(np.abs(ensemble[:, None] - ensemble[None, :]))  # All pairs
