    ...
    **kwargs: Any,
    ...
    Args:
        ...
        **kwargs: Additional arguments for solve_ivp, or — on the
            implicit-midpoint path — for
            :func:`otwin.model.integrators.implicit_midpoint` (``newton_tol``,
            ``max_iter``, ``method``... ; pass ``jac`` there to get the analytic
            Newton path, or use :func:`otwin.model.integrators.integrate_phs`,
            which derives it from the model).
    ...
    # Called with a bare vector field it has no analytic Jacobian to work with,
    # so it finite-differences the residual inside Newton; a caller holding the
    # model itself should prefer `integrate_phs`, which recovers the analytic
    # Jacobian (and, for quadratic H, a closed-form step that is ~100x faster).
