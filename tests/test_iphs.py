...
from otwin.model import IrreversiblePHS
...
def test_satisfies_the_irreversible_contract():
    """An IPHS must satisfy the full port-Hamiltonian contract as well.

    The energy structure is forwarded from the reversible sub-system rather
    than duplicated, so there is exactly one definition of H, J, R and g.
    """
    import numpy as np

    from otwin.interfaces import IrreversibleModel, PortHamiltonianModel, TwinModel

    sys = IrreversiblePHS(
        H=lambda x: float(x[0]),
        S=lambda x: float(x[1]),
        J=lambda x: np.zeros((2, 2)),
        R=lambda x: np.zeros((2, 2)),
        L=lambda x: np.array([[0.0, 0.0], [0.0, 0.5]]),
    ...
    assert isinstance(sys, IrreversibleModel)
    assert sys.entropy_production(np.array([1.0, 1.0])) >= 0.0
